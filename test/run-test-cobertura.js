...
if (!prNumber) {
  console.error('Usage: node test/run-test-cobertura.js <PR_NUMBER>');
  process.exit(1);
}
...
// Set environment variables for the tool - using Cobertura format
process.env.INPUT_BASE_COVERAGE_REPORT_PATH = path.resolve(__dirname, 'reports/base-cobertura.xml');
process.env.INPUT_BRANCH_COVERAGE_REPORT_PATH = path.resolve(__dirname, 'reports/current-cobertura.xml');
...
process.env.INPUT_CUSTOM_MESSAGE = 'This is a test comment from the refactored jest-cov-reporter using Cobertura format';
...
process.env.INPUT_COVERAGETYPE = 'cobertura';  // Changed to cobertura

// Set GitHub context
process.env.GITHUB_REPOSITORY = `${owner}/${repo}`;
process.env.GITHUB_ISSUE_NUMBER = prNumber;

console.log('Running jest-cov-reporter with Cobertura format...');
try {
  // Execute the tool
